...
for i in range(N):
    for j in range(N):
        xij[i,j] = m.addVar(vtype=GRB.BINARY, obj=cij[i][j],name='x_%d_%d' % (i, j))

m.addConstr(quicksum(xij[i,j] for j in range(300)) == 1)                         
...
for j in range(N):
    m.addConstr(quicksum(xij[i,j] for i in range(N)) <= N * xij[j,j])
# fill in constrint

# third set of constrint  ----------------------------
m.addConstr(quicksum(xij[j,j] for j in range(300)) <= K)
# fill in constrint
